...
namespace.module('agherriot.story-generator.form', 
	...
	function validate() {
		...
		for ( var i = 0; i < MAX_NUMBER_OF_CHARACTERS; i++ ) {
			...
			firstname[i] = $("input[name=firstname]").eq(i).val();
			nickname[i] = $("input[name=nickname]").eq(i).val();
			lastname[i] = $("input[name=lastname]").eq(i).val();
			
			shyness[i] = parseInt( $("select[name=shyness]").eq(i).val());
	...
	function initialize() {
	    ...
	    for ( var i = MAX_NUMBER_OF_CHARACTERS-1; i >= 0; i-- ) {
	        ...
	        $("#numberOfCharacters").after( generateCharacterFormText( i ) );
	    ...
	    $("input[type=text][name=firstname]").val( FIRST_NAME_PROMPT );
	    $("input[type=text][name=nickname]").val( NICKNAME_PROMPT );
	    $("input[type=text][name=lastname]").val( LAST_NAME_PROMPT );
	}
	...
	function setUpEventHandlers(){
	    ...
	    $("select[name=numberOfCharacters]").change( function(event) {
	        ...
	        numberOfCharacters = parseInt( $(this).val() );
